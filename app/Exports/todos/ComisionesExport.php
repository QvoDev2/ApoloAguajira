...
class ComisionesExport implements FromArray, WithStyles, WithColumnWidths, WithColumnFormatting, ShouldAutoSize, WithDrawings, WithTitle
{
    public function __construct($filtros)
    {
        ...
        $this->rows = VComision::select(
            ...
            DB::raw("IF (estado=8, (SELECT SUBSTRING_INDEX(observaciones, '-', 1) FROM estados_comisiones WHERE comision_id=vcomisiones.id AND estado=8),'')")
        )
            ->where(function ($q) {
                ...
                if ($this->filtros->zona) $q->whereIn('zona_id', array_values($this->filtros->zona));
                ...
                if (!is_null($this->filtros->novedades)) {
                    $condition = ($this->filtros->novedades == 0) ? '>' : '=';
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('A11:X' . (count($this->rows) + 11))->applyFromArray([
            'borders' => [
                'allBorders' => [
                    'borderStyle' => Border::BORDER_THIN,
                    'color' => ['argb' => '000000'],
        ...
        $sheet->getStyle('A11:X11')->applyFromArray([
            'font' => [
                'bold' => true
            ]
        ]);
    ...
    public function array(): array
    {
        ...
        return array_merge(
            [
                ...
                [''],
                [
                    'Id', 'No. autorización UNP', 'Esquema', 'Cédula del escolta', 'Nombre del escolta',
                    'Fecha aprobación correo', 'Fecha inicio', 'Fecha fin', 'Valor por día',
                    'Días aprobados', 'Valor total días aprobados', 'Días reportados',
                    'Valor total días reportados', 'Origen', 'Puntos de control', 'Viajero', 'Creado por', 'Observaciones',
                    'Estado', 'Fecha inicio comisión', 'Fecha fin comisión', 'Observaciones verificaciones', 'Medio desplazamiento', 'Motivo de rechazo'
                ]
            ],
            $this->rows
        );
