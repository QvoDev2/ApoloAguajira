...
class UpdateVcomisiones8View extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement("
            ...
            SELECT
                ...
                (SELECT GROUP_CONCAT(observaciones SEPARATOR '; ') FROM estados_comisiones e WHERE e.comision_id = c.id AND e.observaciones IS NOT NULL AND e.observaciones != '' GROUP BY e.comision_id) observaciones_verificaciones,
                (SELECT nombre FROM listas md WHERE md.id = c.tipo_desplazamiento_id) medio_desplazamiento
            FROM comisiones c
